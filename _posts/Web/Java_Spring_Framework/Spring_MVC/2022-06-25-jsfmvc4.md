...
1. RequestMappingHandlerMapping: @ReguestMapping 기반의 컨틀롤러
2. BeanNameUrlHandlerMapping: Spring Bean 이름으로 컨틀로 검색

> Handler Adapter

1. RequestMappingHandlerAdapter: @ReguestMapping 기반의 컨틀롤러
...
@RequestMappinbgm이나 @Controller로 명시되어 있는 클래스를 핸들러 매핑정보를 인식한다. 따라서 위처럼 @Controller가 명시되어 있거나
