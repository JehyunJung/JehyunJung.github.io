...
JPQL을 사용해서 대부분의 SQL을 실행할 수 있찜나, 특정 DB에 종속적인 기능들을 수행하기 위해서는 SQL문을 활용해야한다.
...
```
아래와 같이 결과 매핑을 사용해서 쿼리 결과를 받을 수 있다.
...
@SqlResultSetMapping(name="orderResults",
    entities=(
        @EntityResult(entityClass=com.acme.Order.Member.class),fields=(
...
엔티티를 수정하면 기존에는 영속성 컨텍스트에서 변경을 감지해서 수행했는데, 이렇게 진행하게 되면 하나의 엔티티에 대해서 수 백개 이상의 데이터를 처리하는데 시간이 많이 소요된더. 이를 위해 JPQL 기반의 Update/Delete 와 같은 벌크 연산을 수행한다.
...
위의 상황을 보면 플러시를 AUTO 모드로 설정하는 것이 무결성 유지를 할 수 있고, 쿼리 실행 직전 flush를 호출하지 않아서 좋아보이지만, AUTO 모드로 사용하게 되면 플러시가 너무 빈번하게 발생한다는 문제가 있다. 
