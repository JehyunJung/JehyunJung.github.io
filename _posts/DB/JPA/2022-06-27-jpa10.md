...
JPA에서 재공하는 em.find()를 이용해서 특정 엔티티를 검색할 수 있다. 하지만, 가령 나이 30살 이상인 회원 수 는 몇명인가? 와 같은 질의를 하기는 어렵다.

그렇다고, 모든 엔티티를 메모리에 올려서 30살 이상인 회원만 필터링하는 것은 비효율적이다.이럴때는 DB가 제공해주는 SQL문을 활용해야하는 데, JPA에서는 객체 지향적으로 SQL문 쿼리를 할 수 있도록 제공한다.
...
1. 엔티티와 속성는 대소문자 구분이 된다, SQL 문법(SELECT, FROM 과 같은 예약어)는 대소문자 구분이 없다.
2. 테이블 대상이 아니라 엔티티명을 명시한다.
3. 별칭을 사용해야 한다.
...
```
위 처럼, new 명령어를 사용해서 쿼리 수행시 객체 변환작업이 자동적으로 이뤄어 질 수 있도록 할 수 있다. 또 이렇게 타입이 명확해지므로, TypedQuery를 활용할 수 있다.
...
1. 별칭을 줄 수 없다.
    - 별칭 사용을 통한 잘못된 데이터릐 저장을 방지하기 위함
