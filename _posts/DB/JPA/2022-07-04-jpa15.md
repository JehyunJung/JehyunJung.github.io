...
1번의 출력결과는 java.util.Collection이다 하지만, 영속 상태로 저장하게 되면 Hibernate에서 PersistenceBag라는 타입으로 wrapping해서 내보낸다.
...
중복된 엔티티가 있는 지 여부를 확인하지 않아도 되므로 식별자 값 확인을 위한 컬렉션 초기화가 발생하지 않는다.
...
1. 우선, POSITION 관련 부분은 연관관계의 주인이 가지고 있지 않고 일대다에서 일에 해당하는 엔티티에 저장된다. 따라서, 다쪽 엔티티에서는 해당 순서값을 알지 못하므로 저장할때는 POSITION에 null 값이 저장되고, 이후에 UPDATE SQL을 통해 초기화 된다.
...
*글로벌 설정에서 제외하고 싶거나 상속 받은 컨버터를 사용하지 않으려면 @Convert의 disableConversion 속성을 false로 설정하면 된다.*
...
```
위와 같이 하면, order2조회하는 시점에는 이미 영속성 컨텍스트에 해당 엔티티가 있어서 엔티티 그래프를 수행하지 않고 이전에 로딩된 엔티티가 반환된다.
