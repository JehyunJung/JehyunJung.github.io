...
공용 회선 하나를 이용해서 여러 개의 node가 네트워크를 공유하고 있는 구조이면, LAN을 구성할 때 주로 사용하는 방식이다.
...
p2p의 경우 client 간의 통신 과정에 있어 server의 매개가 필요없고 client 간에 다이렉트으로 연결되어 통신을 수행하게 된다.
...
3-way handshaking 과정을 통해 seq#, buffer size와 같은 각종 parameter을 처리한다. 
